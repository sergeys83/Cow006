...
public class Utils : MonoBehaviour
{
	...
	public List<CardCowMover> sortedCardCows(List<CardCowMover> tempList)
	{
		var _cardsT = new List<CardCowMover>(tempList).ToArray();
		_cardsT = tempList.OrderBy(item => item.cv.Attack).ToArray();
		return new List<CardCowMover>(_cardsT);
	}
	public  int MinPositive(int[] array)
	{
		var minPositiveNumber = -1;
		var minPositiveNumberIndex = -1;
		...
		for (var i = minPositiveNumberIndex + 1; i < array.Length; i++)
		{
	...
	public int MinId(CardCowMover card, List<Transform> cardsTr, TargetRowFinder _row)
	{
		var Atak = card.cv.Attack;
		var g = new int[4];
		for (var i = 0; i < cardsTr.Count; i++)
		{
			_row = cardsTr[i].GetComponent<TargetRowFinder>();
			g[i] = Atak - _row.lastCard().cv.Attack;
		}

		int x = g.Max();
		return x;
	}
	public static int MinIdNet(CardNet card, List<Transform> cardsTr, RawViewer _row)
	{
		var Atak = card.Attack;
		var g = new int[4];
		for (var i = 0; i < cardsTr.Count; i++)
		{
	...
	public static List<CardNet> UpgradeCardList(List<CardView> lCard)
	{
		var lCC = new List<CardNet>();
		foreach (var item in lCard) lCC.Add(item as CardNet);
		return lCC;
	}
//============================ Bounds Functions ============================\
